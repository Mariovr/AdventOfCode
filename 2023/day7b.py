...
class Hand(object):
    ...
    def det_type(self):
        ...
        if 'J' in self.all_freq.keys():
            save_j = self.all_freq['J']
            if self.all_freq['J'] != 5:
                del(self.all_freq['J'])
                li = [ (key, value ) for key, value in self.all_freq.items() ]
                li.sort(key = lambda x : x[1])
                self.all_freq[li[-1][0]] += save_j
